...
4. define um **radix customizado `sseg`**, que mostra na waveform o _dígito que aparece no
   display_ em vez do padrão de bits dos segmentos;
...
_Conversão de `70.5`: a página devolve `(0x8D / 256) × 2⁷`, classifica como **exato** e lista
os quatro passos de chaves. Abaixo, os painéis dos operandos bit a bit, a réplica dos displays
com o valor decimal, e o detalhamento estágio por estágio._
...
_Reprodução de `debug/sim_demo.vcd` no instante 330 ns (evento 10 de 147). Os displays mostram
...
à fonte. _Correção:_ extraímos os módulos diretamente do texto do PDF.
...
acendiam padrões sem sentido. _Correção:_ consultamos o manual da placa, confirmamos
...
sem reinício. _Correção:_ `KEY0` carrega o campo apontado por `SW(9:8)`, e `KEY1` virou
...
ambígua. _Correção:_ adotamos um número por vez, com `SW(7)` alternando o display, e usamos os
...
com o sinal preservado.** _Correção:_ mantivemos `sign_out <= signb` como está. A própria IA
...
valores originais. _Correção humana:_ "Você deveria ter mantido o expoente e representado a
...
- **Manter `fp_adder.vhd` intocado.** Decisão nossa, verificada por comparação de hash ao
  longo da sessão. A IA chegou a _propor_ um underflow gradual (`sh := minimum(leado, expb)`)
...
| ------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Multiplexar a entrada por campo** (`SW(9:8)` seleciona, `KEY0` carrega) | Testamos o argumento contra a alternativa do contador nos botões e ele se sustentou: a carga por chaves é _idempotente_, então repetições causadas pelo quique do botão são inofensivas. Um contador não tem essa propriedade     | Que uma escolha de interface pode **eliminar** um problema de hardware (debounce) em vez de tratá-lo. Passamos a olhar a idempotência como propriedade de projeto, não como detalhe                          |
